...
export default function UploadingStatus() {
  ...
  const { uploadFinished, fileStatuses, skippedFiles, hasPermissionError } = useSelector(
    state => state.upload,
  );
    ...
    if (uploadFinished) {
      ...
      if (hasFileFailed) {
        if (hasPermissionError) {
          toastError('You have read-only permission for this bucket');
        } else {
      ...
      if (hasPermissionError) {
        dispatch(setHasPermissionError(false));
      }
  ...
  }, [dispatch, hasFileFailed, hasPermissionError, uploadFinished, skippedFiles, toastSuccess, toastError]);
