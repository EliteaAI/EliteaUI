...
const emptyListPlaceHolder = <div>No pipelines are currently pending approval.</div>;

export default function ModerationPipelineList({ setTabCount }) {
  ...
  }, [isFetching, loadMorePipelines]);

  const realDataList = React.useMemo(() => {
    const prompts = applications.map(i => ({
      ...i,
      cardType: ContentType.ModerationSpacePipeline,
    }));
