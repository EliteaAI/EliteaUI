...
const EditEntityModal = memo(props => {
  ...
  const initialPhase = skipPrompt ? PHASES.LOADING : PHASES.PROMPT;
  ...
  const handleClose = useCallback(() => {
    ...
    resetGenerate();
    onClose();
  }, [onClose, resetGenerate, initialPhase]);

  const handleGenerate = useCallback(async () => {
    if (!skipPrompt && !description.trim()) return;
    ...
    try {
      ...
      generatePromiseRef.current = null;
      // Without a prompt to go back to there is nowhere to return to but a retryable error.
      setPhase(skipPrompt ? PHASES.ERROR : PHASES.PROMPT);
    }
  }, [description, onGenerate, resetGenerate, onDraftGenerated, skipPrompt]);

  // Prompt-free callers generate as soon as the modal opens. The ref keeps that to once per
  // open: handleGenerate's identity changes with every draft callback the caller re-creates.
  useEffect(() => {
    if (!open || !skipPrompt) {
    ...
    handleGenerate();
  }, [open, skipPrompt, handleGenerate]);
  ...
  const isSaveDisabled = isBusy || !isDraftValid || saveDisabled;

  const generateErrorMessage =
    generateError?.data?.error || generateError?.data?.detail || 'Failed to generate. Please try again.';
    ...
    if (phase === PHASES.LOADING) {
      return (
        ...
        >
          ...
          >
            {loadingText || `Generating ${entityLabel} draft...`}
          </Typography>
        </Box>
      );
    }
            ...
            <>
              ...
              {onSaveAsVersion && (
                <BaseBtn
                  ...
                  disabled={isSaveDisabled || saveAsVersionDisabled}
                >
