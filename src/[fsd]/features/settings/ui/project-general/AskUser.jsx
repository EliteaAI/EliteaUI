...
const AskUser = memo(() => {
  ...
  const handleToggle = useCallback(
    (event, checkedValue) => {
      ...
      setFieldValue('default_ask_user_enabled', checkedValue);
      requestSubmit();
    },
  ...
  return (
    ...
    >
      <EnableToggleCard
        enabled={values.default_ask_user_enabled}
        onToggle={handleToggle}
        disabled={!canEditProjectContext}
        title="Ask User (Clarifying Questions)"
        description="Allow agents to pause and ask clarifying questions before continuing."
      />
    </Box>
  );
});

AskUser.displayName = 'AskUser';
