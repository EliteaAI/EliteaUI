...
const uniqueAuthorizationRequests = requests => {
  ...
  requests.forEach(request => {
    const id = request?.authorization_request_id || request?.interrupt_id || request?.tool_run_id;
...
export const buildMcpAuthorizationToolAction = ({
  ...
  const requestId = metadata?.authorization_request_id || metadata?.interrupt_id || metadata?.tool_run_id;
...
const getToolkitNameFromAction = action => {
  if (typeof action?.name === 'string') {
    if (action.name.startsWith(MCP_AUTHORIZE_PREFIX)) {
      return action.name.slice(MCP_AUTHORIZE_PREFIX.length);
    }
    return action.name;
  }
  return null;
...
export const getToolkitAuthorizationContext = action => {
  ...
  const nameFromAction = getToolkitNameFromAction(action);
  const toolkitName =
