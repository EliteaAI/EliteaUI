...
const attachmentButtonStyles = isDisabled => ({
  ...
  attachIcon: ({ palette }) => ({
    fontSize: '1rem',
    '& path': {
      fill: isDisabled ? palette.icon.fill.disabled : palette.icon.main,
    },
  }),
  label: ({ palette }) => ({
    color: isDisabled ? palette.text.button.disabled : palette.text.secondary,
    flex: 1,
    textAlign: 'left',
  }),
  counter: ({ palette }) => ({
    color: isDisabled ? palette.text.button.disabled : palette.text.default,
    flexShrink: 0,
  }),
});
