...
const getStyles = (isRecording, disabled) => {
  const micButtonColor = palette =>
    disabled ? palette.icon.disabled : isRecording ? palette.icon.accent : palette.icon.secondary;
  return {
    wrapper: {
      display: 'flex',
      alignItems: 'center',
      gap: isRecording ? '0.5rem' : '0rem',
      border: isRecording ? theme => `0.0625rem solid ${theme.palette.primary.main}` : 'none',
      padding: '0rem',
